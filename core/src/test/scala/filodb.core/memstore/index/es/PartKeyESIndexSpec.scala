...
import filodb.core.memstore.{PartKeyLuceneIndex, PartKeyLuceneIndexRecord, TermInfo}
...
class PartKeyESIndexSpec  extends AnyFunSpec with Matchers with BeforeAndAfter {
  import Filter._
  import GdeltTestData._


  val keyIndex = new PartKeyESIndex(dataset6.ref, dataset6.schema.partition, 0, 1.hour.toMillis)
  ...
  // Exposes Lucene API, work on this after abstracting the part key access
...
//  }
  ...
  it("should obtain indexed names and values") {
    ...
    keyIndex.indexValues("not_found") should equal (Nil)
